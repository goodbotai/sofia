...
function pickConversation(err, convo, {language: lang, name}, bot) {
  ...
  psed(err, convo, rapidProContact);
  dmc(err, convo, rapidProContact);
  caregiverKnowledge(err, convo, language);

  convo.addQuestion(generateQuickReply('Sofia trial pick a survey',
                                           ['FCI',
                                            'Caregiver Knowledge',
                                            'SRQ 20',
                                            'DMC 3',
                    ...
                    [{
                      ...
                      pattern: /dmc*/ig,
                      callback: (res, convo) => {
                        idString = getFormId('DMC');
                        convo.gotoThread('dmc');
                        convo.next();
                      },
                    }, {
                      pattern: /psed*/ig,
                      callback: (res, convo) => {
                        idString = getFormId('PSED');
                        convo.gotoThread('psed');
                        convo.next();
                      },
                    }, {
                      default: true,
